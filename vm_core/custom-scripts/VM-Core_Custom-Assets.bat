set LOGFILE=%temp%\VM-Core_Custom-Assets.log
echo Expanding VM-Core_Custom-Assets.cab >> %LOGFILE%
expand -r .\VM-Core_Custom-Assets.cab -F:* . >> %LOGFILE%
...
echo result: %ERRORLEVEL% >> %LOGFILE%
echo . >> %LOGFILE%
echo VM Core Custom Assets Installed >> %LOGFILE%