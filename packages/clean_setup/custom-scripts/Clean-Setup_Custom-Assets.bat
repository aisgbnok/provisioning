set LOGFILE=%temp%\Clean-Setup_Custom-Assets.log
echo Expanding Clean-Setup_Custom-Assets.cab >> %LOGFILE%
expand -r .\Clean-Setup_Custom-Assets.cab -F:* . >> %LOGFILE%
...
echo result: %ERRORLEVEL% >> %LOGFILE%
echo . >> %LOGFILE%
echo Clean Setup Custom Assets Installed >> %LOGFILE%